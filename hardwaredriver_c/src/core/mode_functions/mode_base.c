...
ErrorCode mode_base_create(ModeBase** mode, SerialInterface*serial_interface, 
    ...
    if (!mode || !serial_interface || !process_manager || !vtable || !impl) {
    ...
    new_mode->serial_interface =serial_interface;
...
ErrorCode mode_base_handshake(ModeBase* mode) {
    if (!mode || !mode->serial_interface) {
        return ERROR_INVALID_PARAM;
    }

    // Reset hardware at 60Hz
    ErrorCode result = serial_interface_reset_hardware(mode->serial_interface, true);
    ...
    result = serial_interface_write_data(mode->serial_interface, &mode_byte, 1);
    if (result != ERROR_NONE) {
    ...
    if (config && config_len > 0) {
        result = serial_interface_write_data(mode->serial_interface, config, config_len);
        if (result != ERROR_NONE) {
    ...
    uint8_t device_byte;
    size_t bytes_read;
    result = serial_interface_read_data(mode->serial_interface, &device_byte, &bytes_read, 1);
...
ErrorCode mode_base_flush_data(ModeBase* mode) {
    ...
    while (total_bytes < MAX_FLUSH_BYTES) {
        size_t bytes_read;
        ErrorCode result = serial_interface_read_data(mode->serial_interface, buffer, &bytes_read, READ_CHUNK_SIZE);
